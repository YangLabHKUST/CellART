...
def main():
    ...
    for name in tqdm(names):
        ...
        if gt_path.endswith('.tif'):
            ...
            raise ValueError('The suffix of label name should be _label.tiff or _label.npy')
        if seg_path.endswith('.tif'):
            seg = tif.imread(seg_path)
        elif seg_path.endswith('.npy'):
            seg = np.load(seg_path)
        else:
            raise ValueError('The suffix of label name should be _label.tiff or _label.npy')
        if gt.shape[0] > seg.shape[0] or gt.shape[1] > seg.shape[1]:
            temp = np.zeros_like(gt)
            temp[:seg.shape[0], :seg.shape[1]] = seg
            seg = temp
        # Score the cases
        ...
        seg_metric['Jaccard_Index'].append(np.round(jac, 4))
    ...
    print('mean Jaccard Index:', np.mean(seg_metric['Jaccard_Index']))
